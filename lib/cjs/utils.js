"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Utils = void 0;
const core_1 = require("@aws-cdk/core");
const config_loader_1 = require("./config-loader");
class Utils {
    static async getConfig(configDir, configBase, configEnv) {
        const loader = new config_loader_1.ConfigLoader(configDir, configBase);
        return await loader.load(configEnv);
    }
    static async run(app, configDir, stack, props) {
        var _a, _b, _c;
        const configEnv = (_a = props === null || props === void 0 ? void 0 : props.configEnv) !== null && _a !== void 0 ? _a : app.node.tryGetContext('env');
        let config = await this.getConfig(configDir, props === null || props === void 0 ? void 0 : props.configBase, configEnv);
        core_1.Tags.of(app).add('College', config.College);
        core_1.Tags.of(app).add('Environment', config.Environment);
        const mainStackName = this.getMainStackName(config);
        return new stack(app, mainStackName, {
            env: {
                account: (_b = config === null || config === void 0 ? void 0 : config.AWSAccountId) !== null && _b !== void 0 ? _b : process.env.CDK_DEFAULT_ACCOUNT,
                region: (_c = config === null || config === void 0 ? void 0 : config.AWSRegion) !== null && _c !== void 0 ? _c : process.env.CDK_DEFAULT_REGION
            }
        }, config, props === null || props === void 0 ? void 0 : props.idSuffix);
    }
    static getMainStackName(config) {
        return `${this.getBaseName(config)}-${config.Name}`;
    }
    static getBaseName(config) {
        return `${config.College.toLowerCase()}-${config.Environment.toLowerCase()}`;
    }
}
exports.Utils = Utils;
//# sourceMappingURL=utils.js.map